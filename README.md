...
## ⚡ Zarządzanie Połączeniami Bazy Danych (asyncpg Connection Pool)

W środowiskach produkcyjnych otwieranie osobnego połączenia (`asyncpg.connect()`) dla każdego żądania HTTP przy dużym ruchu prowadzi do natychmiastowego wyczerpania limitu połączeń PostgreSQL i błędów HTTP 500.

W tym projekcie zastosowano **Pulę Połączeń (`asyncpg.Pool`)** udostępnianą w cyklu życia aplikacji FastAPI (`lifespan`):
...
async def lifespan(app: FastAPI):
    # Utworzenie puli połączeń przy starcie aplikacji
    app.state.db_pool = await asyncpg.create_pool(
        dsn="postgresql://user:password@localhost:5432/returns_db",
        min_size=5,
        max_size=20,
        timeout=10.0
    )
    yield
    # Bezpieczne zamknięcie puli przy zamykaniu serwera
    await app.state.db_pool.close()

app = FastAPI(lifespan=lifespan)
```

W narzędziach agenta i endpointach połączenia są bezpiecznie pobierane z puli:

```python
async def verify_order_in_db(order_id: str, pool: asyncpg.Pool) -> dict | None:
    async with pool.acquire() as connection:
        row = await connection.fetchrow(
            "SELECT id, status, total_amount FROM orders WHERE id = $1", order_id
        )
